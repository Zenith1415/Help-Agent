...
> *We deliberately choose NOT to retroactively relabel these disputed cases.* Post-hoc editing of test labels after inspecting model errors introduces acute observer bias and invalidates benchmark integrity. Instead, we preserve the golden set strictly frozen as-is and document this finding as a primary known limitation: **lexical seed sampling biases ground truth in favor of bag-of-words models and penalizes pragmatic semantic understanding**. Crucially, the fix is not hypothetical: the exact same embedding-cluster approach already built in `data/cluster_intents.py` for taxonomy discovery should also drive candidate sampling for the golden set, rather than the keyword-seeded approach used this round, combined with multi-annotator consensus.
>
